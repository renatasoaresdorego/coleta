...
(
    ...
    CONSTRAINT pk_t_rec_ponto PRIMARY KEY (id_ponto_coleta)
);

CREATE TABLE t_rec_ponto_agendamentos_de_coleta
(
...
ALTER TABLE t_rec_ponto_agendamentos_de_coleta
    ADD CONSTRAINT uc_trecpontoagendamentosdecoleta_agendamentosdecoletaidagenda UNIQUE (agendamentos_de_coleta_id_agenda);
...
ALTER TABLE t_rec_agenda
    ADD CONSTRAINT FK_T_REC_AGENDA_ON_PONTODECOLETA_ID_PONTO_COLETA FOREIGN KEY (ponto_de_coleta_id_ponto_coleta) REFERENCES t_rec_ponto (id_ponto_coleta);

ALTER TABLE t_rec_caminhao
    ADD CONSTRAINT FK_T_REC_CAMINHAO_ON_ID_ROTA FOREIGN KEY (id_rota) REFERENCES t_rec_rota (id_rota);

ALTER TABLE t_rec_ponto_agendamentos_de_coleta
    ADD CONSTRAINT fk_trecponagedecol_on_agenda FOREIGN KEY (agendamentos_de_coleta_id_agenda) REFERENCES t_rec_agenda (id_agenda);

ALTER TABLE t_rec_ponto_agendamentos_de_coleta
    ADD CONSTRAINT fk_trecponagedecol_on_ponto_de_coleta FOREIGN KEY (ponto_de_coleta_id_ponto_coleta) REFERENCES t_rec_ponto (id_ponto_coleta);
